...
import (
	...
	testutils "github.com/8adimka/Go_AI_Assistant/tests/testing"
...
func TestServer_DescribeConversation(t *testing.T) {
	...
	t.Run("describe existing conversation", testutils.WithFixture(func(t *testing.T, f *testutils.Fixture) {
		c := f.CreateConversation()
		srv := chat.NewServer(f.Repository, nil, nil)
	...
	t.Run("describe non existing conversation should return 404", testutils.WithFixture(func(t *testing.T, f *testutils.Fixture) {
		srv := chat.NewServer(f.Repository, nil, nil)
...
func TestServer_StartConversation(t *testing.T) {
	...
	t.Run("successfully starts conversation with valid message", testutils.WithFixture(func(t *testing.T, f *testutils.Fixture) {
		...
		srv := chat.NewServer(f.Repository, mockAssist, nil)
	...
	t.Run("returns error for empty message", testutils.WithFixture(func(t *testing.T, f *testutils.Fixture) {
		mockAssist := &MockAssistant{
	...
	t.Run("handles title generation error gracefully", testutils.WithFixture(func(t *testing.T, f *testutils.Fixture) {
	...
	t.Run("returns error when reply generation fails", testutils.WithFixture(func(t *testing.T, f *testutils.Fixture) {
	...
	t.Run("handles whitespace-only message as empty", testutils.WithFixture(func(t *testing.T, f *testutils.Fixture) {
		mockAssist := &MockAssistant{
			TitleResponse: "Test Title",
			ReplyResponse: "Test Reply",
		}
